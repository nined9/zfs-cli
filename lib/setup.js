 ...
 * @LastEditors: 黎加冬
 * @LastEditTime: 2020-12-02 10:58:16
 * @Description: 一键安装依赖包
...
function walk(packagePath, packageLockPath, {libs, originRegistry: normalRegistry}) {
  let pkgJsonStr = "";
  let pkgLockJsonStr = "";
  try {
  ...
  try {
    pkgLockJsonStr = fs.readFileSync(packageLockPath, {encoding: "utf-8"});
  } catch (e) {
  }
  ...
  if (flag) {
    ...
    try {
      ...
      fs.writeFileSync(packageLockPath, pkgLockJsonStr, {encoding: "utf-8"});
      return;
    }
    ...
    Object.keys(libs).forEach(registry => {
      ...
      const installPkgs = packages.reduce((installPkgs, package) => {
