 ...
 * @LastEditors: 黎加冬
 * @LastEditTime: 2020-09-10 18:16:51
 * @Description: 一键安装依赖包
...
function walk(packagePath, configJson) {
  let pkg = "";
  try {
    pkg = fs.readFileSync(packagePath, {encoding: "utf-8"});
  } catch (e) {
  ...
  Object.keys(configJson).forEach(registry => {
    const packages = configJson[registry];
  ...
  if (flag) {
    ...
    try {
      // console.log("正在将您的npm源切换为：" + taobaoRegistry);
      cp.execSync("npm config set registry " + taobaoRegistry, {stdio: 'inherit'});
      // console.log("切换【" + taobaoRegistry + "】源成功...");
